...
const legalRoutes = ["/privacidad", "/terminos", "/cookies", "/aviso-legal"];

export default function Footer() {
  const { t } = useLanguage();
  const f = t.footer;
  const navLabels = [t.nav.home, t.nav.about, t.nav.services, t.nav.cases, t.nav.faq, t.nav.contact, t.nav.emergency];
  const serviceLabels = t.services.items.slice(0, 6).map((s) => s.title);
    ...
    <footer className="bg-[#0a0a0a] border-t border-white/5 relative overflow-hidden">
      ...
      <div className="bg-red-900/30 border-b border-red-800/30 py-4 px-4">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-white font-bold text-sm tracking-wide">{f.emergency.label}</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            <Link href="/" className="flex items-center gap-3 group mb-6">
              ...
              <div>
                <div className="text-white font-black text-2xl" style={{ fontFamily: "'Bebas Neue', Impact, sans-serif", letterSpacing: "0.05em" }}>
                  YOUSSEF OK
                </div>
                <div className="text-red-500 text-[9px] font-bold tracking-[0.2em] uppercase">OKUPA KILLER</div>
              </div>
            </Link>
            <p className="text-gray-400 text-sm leading-relaxed mb-6">{f.tagline}</p>
            <div className="flex items-center gap-3">
              {[{ icon: Instagram, href: "#", label: "Instagram" }, { icon: Facebook, href: "#", label: "Facebook" },
                { icon: Twitter, href: "#", label: "Twitter" }, { icon: Youtube, href: "#", label: "YouTube" }].map(({ icon: Icon, href, label }) => (
          ...
          <div>
            <h3 className="text-white font-bold text-sm uppercase tracking-[0.15em] mb-5 flex items-center gap-2">
              <span className="w-6 h-px bg-red-500" />{f.sections.services}
            </h3>
            <ul className="space-y-3">
              {serviceLabels.map((label, i) => (
                <li key={i}>
                  <Link href={serviceLinks[i].href}
          ...
          <div>
            <h3 className="text-white font-bold text-sm uppercase tracking-[0.15em] mb-5 flex items-center gap-2">
              <span className="w-6 h-px bg-red-500" />{f.sections.nav}
            </h3>
            <ul className="space-y-3">
          ...
          <div>
            <h3 className="text-white font-bold text-sm uppercase tracking-[0.15em] mb-5 flex items-center gap-2">
              <span className="w-6 h-px bg-red-500" />{f.sections.contact}
            </h3>
            <ul className="space-y-4">
              <li>
                <a href="tel:+34600893591" className="flex items-start gap-3 group">
                  ...
                  <div>
                    <div className="text-gray-500 text-xs mb-0.5">{f.contact.phone.label}</div>
                    <div className="text-white text-sm font-semibold group-hover:text-red-400 transition-colors duration-200">+34 600 89 35 91</div>
                  </div>
                </a>
              </li>
              <li>
                <a href="mailto:info@youssefok.es" className="flex items-start gap-3 group">
                  <div className="w-8 h-8 bg-red-600/10 border border-red-600/20 rounded-lg flex items-center justify-center flex-shrink-0 group-hover:bg-red-600/20 transition-colors duration-200">
                    <Mail className="w-3.5 h-3.5 text-red-400" />
                  </div>
                  <div>
                    <div className="text-gray-500 text-xs mb-0.5">{f.contact.email.label}</div>
                    <div className="text-white text-sm font-semibold group-hover:text-red-400 transition-colors duration-200">info@youssefok.es</div>
                  </div>
                </a>
              </li>
              <li className="flex items-start gap-3">
                ...
                <div>
                  <div className="text-gray-500 text-xs mb-0.5">{f.contact.coverage.label}</div>
                  <div className="text-white text-sm font-semibold">{f.contact.coverage.value}</div>
                </div>
              </li>
            </ul>

            <div className="mt-6 p-4 bg-green-900/20 border border-green-700/30 rounded-xl">
              <a href="https://wa.me/34600893591" target="_blank" rel="noopener noreferrer"
                ...
                <span className="text-sm font-medium">{f.contact.whatsapp}</span>
              </a>
            </div>
          </div>
        </div>
      ...
      <div className="border-t border-white/5 py-6 px-4">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-gray-500 text-sm">© {currentYear} Youssef OK. {f.copyright}</p>
          <div className="flex items-center gap-6">
            {f.legal.map((label, i) => (
              <Link key={i} href={legalRoutes[i]}
